...
parser.add_argument('--model', type=str, default='GCN_subatt',help='Model string')
####################### additional #############################
...
for i in range(count):
    adj, features, labelall = tuple(pkl.load(open('../data/' + args.dataset + '_data.pkl','rb')))
    ...
    struc_feat = np.load('../data/' + args.dataset + '_' + args.struc_features + '.npy')
    ...
    if args.model == 'GCN_subatt':
        model = GCN_subatt(nfeat=features.shape[1],
                       nhid=args.hidden1,
                       nclass=labels.max().item() + 1,
                       dropout=args.dropout,
                       nstruc=struc_feat.shape[1])

    elif args.model == 'GCN_subatt_test':
        model = GCN_subatt_test(nfeat=features.shape[1],
                       nhid=args.hidden1,
                       nclass=labels.max().item() + 1,
                       dropout=args.dropout,
                       nstruc=struc_feat.shape[1])
    elif args.model == 'GAT_subatt':
        model = GAT_subatt(nfeat=features.shape[1],
                      nhid=args.hidden1,
                      nclass=labels.max().item() + 1,
                      dropout=args.dropout,
                      nstruc=struc_feat.shape[1],
    ...
    def train(epoch):
        ...
        optimizer.zero_grad()
        output_supervised, output_unsupervised,al = model(features, adj)

        # モデル側の返り値が既にsoftmaxであるのでnll_loss関数を使用してクロエントロピーを出している。
        # F.mse_loss = 平均二乗誤差

        su_loss = F.nll_loss(output_supervised[idx_train], labels[idx_train])

        al = torch.sqrt(al)
        tmp_uns = al * output_unsupervised
        tmp_struc = al * struc_feat
        uns_loss = F.mse_loss(tmp_uns,tmp_struc)

        loss_train = (1 - alpha) * su_loss + alpha * uns_loss
        ...
        if not args.fastmode:
            ...
            su_loss_train = F.nll_loss(output_supervised[idx_val], labels[idx_val])
            al = torch.sqrt(al)
            tmp_uns = al * output_unsupervised
            tmp_struc = al * struc_feat
            uns_loss_train = F.mse_loss(tmp_uns, tmp_struc)

        loss_val = (1 - alpha) * su_loss_train + alpha * uns_loss_train
    ...
    def test():
        ...
        output_supervised, output_unsupervised,al = model(features, adj)
        su_loss_train = F.nll_loss(output_supervised[idx_test], labels[idx_test])
        al = torch.sqrt(al)
        tmp_uns = al * output_unsupervised
        tmp_struc = al * struc_feat
        uns_loss_train = F.mse_loss(tmp_uns, tmp_struc)

        loss_test = (1 - alpha) * su_loss_train + alpha * uns_loss_train
