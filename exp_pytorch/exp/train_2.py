...
for i in range(count):
    ...
    #adj = adj + sp.eye(adj.shape[0]) #正規化やめると精度が上がるかも(多分上がらない）
    #print(torch.sum(adj))
    ...
    def train(epoch):
        ...
        output_supervised, output_unsupervised,output_sub = model(features, adj)  # 順伝播
        ...
        sub_loss_train = F.mse_loss(output_sub,output_unsupervised)

        loss_train = (1 - alpha) * su_loss_train + alpha * uns_loss_train + beta * sub_loss_train
        ...
        if not args.fastmode:
            ...
            model.eval()
            output_supervised, output_unsupervised,output_sub = model(features, adj)

        loss_val = (1 - alpha) * F.nll_loss(output_supervised[idx_val], labels[idx_val]) \
            + alpha * F.mse_loss(output_unsupervised, struc_feat) + beta * F.mse_loss(output_sub,output_unsupervised)
    ...
    def test():
        ...
        loss_test = (1 - alpha) * F.nll_loss(output_supervised[idx_test], labels[idx_test]) + alpha * F.mse_loss(
            output_unsupervised, struc_feat) + beta * F.mse_loss(output_sub,output_unsupervised)
